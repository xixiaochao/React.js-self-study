...
// M :model  :数据层  V： view  视图层 C: controller 控制器
...
//跑通react
//cd day1
//yarn start :跑起环境，react会自动开启一个本地端口并且打开浏览器，把页面渲染出来

//node_modules：存放一些模块，有第三方模块；
//public:存放一个html，webpack将最后打包的js，会自动插入这个html中；React会获取到当前页面中id叫root元素
...
//如果webpack需要支持less，sass；需要更改配置文件
//less-loader  去配置 需要将配置文件暴露出来，需要执行yarn eject 命名：但是这个命令，是不可逆的

//1.在react的结构中，不许出现相对路径，因为webpack在进行解析时，不能够成功对这样的路径进行解析

//vue全家桶 vue + vue-router + vue-cli + axios + vuex + webpack + element iview
//react全家桶 create-react-app react react-router react-redux redux react-dom ant/dva
